...
def read_data(args):
    Data_Dir = args.Data_Dir; N = args.data_res; Nx, Ny, Nz = N, N, N
    start = args.start; seq_length = args.seq_length
    dt = 0.0025; T = 5*10**1; Nt = int(T/dt); K = 20; del_t = 2*K*dt; nu = 0.006416

    data_train = np.zeros([seq_length,3,Nz,Ny,Nx], dtype = np.float32)
    for q in range(start,seq_length):
        ...
        data_train[q:q+1,:,:,:,:] = fr[key][:]
        fr.close()
    data_train = torch.from_numpy(data_train[start:,:,:,:,:])
    
    data_test = np.zeros([seq_length,3,Nz,Ny,Nx], dtype = np.float32)
    for q in range(start,seq_length):
        ### Read data ###
        ReadData = Data_Dir + f"/Training data/Train{1} time{q*del_t:.1f} 3D FHIT nu={nu} n={N} T={T} dt={dt:.4f} K={2*K} data.h5"
        fr = h5py.File(ReadData, 'r')
        key = f"vel{q*del_t:.1f}" if q > 0 else f"vel{0}"
        data_test[q:q+1,:,:,:,:] = fr[key][:]
        fr.close()
    data_val = torch.from_numpy(data_test[start:start+(seq_length - start)//2,:,:,:,:])
    data_test = torch.from_numpy(data_test[start+(seq_length - start)//2:,:,:,:,:])
    print(data_train.shape, data_val.shape, data_test.shape)
    return data_train, data_val, data_test

def get_batch(data, args, shuffle=True):
    batch_size = args.batch_size; length = data.shape[0]

    # data /= torch.std(data, dim=(-3,-2,-1), correction=0, keepdim=True)
    dataloader = DataLoader(data, batch_size=batch_size, shuffle=shuffle, num_workers=8)
    return dataloader
